...
- **Status**: v0.3a release gate passed for `3.2.0-beta.1`.
- **Phase progress**: Phase 0 spike waived; Phase 1, Phase 2, and Phase 3 required scope completed. Long-term observation gates are closed by product decision and are not release blockers.
...
**完成一次远程变更所需的操作摩擦**。当前 release 以本地 AI chat、plan card、逐步确认、snapshot、rollback 和 Never-list 安全约束为主验收对象。
...
- founder 承担"E4B 能胜任 planning"的先验风险，gate 后移到 release validation
- 原 spike / dogfood / keystroke 观察类判据已关闭，不再作为 release gate 的 blocker

**接受的代价**：若 v0.1 release validation 发现 E4B 不达标，已投入的 Phase 1 代码（chat / scrubber / health check / runtime）仍可用；损失约等于 1 个 Phase 1 实现周。接受。

### Phase 1 — v0.1 α scope (3 周)

Chat panel + 凭证 scrub + 命令候选（不自动回车）。
...
v0.3a 必需项全绿后进入 ship；v0.3b 按剩余预算追加。
...
- **进程端口约束**：TunnelFiles 进程全程不监听任何 TCP/UDP 端口，验证时 `netstat -an | grep LISTEN | grep <pid>` 必须为空。
...
Phase 0 spike 已关闭，不保留待办 checkbox。

### v0.1 验收

- 凭证 scrub 单测：完成。
- 输入框 entropy 实时检测单测：完成。
- `ai_health_check` 四态测试：完成。
- chat 流式中断恢复测试：完成。
- keystroke benchmark：长期观察类 gate，已关闭。

### v0.2 验收

- allowlist bypass 单测：完成。
- prompt injection 回归：完成。
- probe session 生命周期测试：完成。
- probe 认证失败计数隔离测试：完成。
- ts-rs regen 同步：完成。

### v0.3 验收

- 端到端 demo：「给 nginx 加 gzip 并验证」整个流程跑通，用户已手工确认。
- rollback 失败路径测试：完成。
- write step 故意改错测试：完成。
- snapshot 磁盘满测试：完成。
- 大文件警告测试：完成。
- plan 滚动修订测试：完成。
...
- `netstat` 验证 TunnelFiles 进程无任何 LISTEN 端口 + 无外部 HTTP：用户已手工确认。
- AI 默认关闭：完成。
- 三阶段验证：完成。
...
3. 合回 AI 冲刺分支
4. 在 release notes 或 issue tracker 记录 slippage（天数），但**不重置** 12 周倒计时
...
## 11. Dogfood Gate（closed）

长期观察类 gate 已由产品决策关闭：不要求 T-1 baseline、keystroke benchmark、14 天 dogfood log、dogfood retro 或 post-hoc spike proxy signals。

保留的发布判断以 checkpoint rubric、自动化测试、用户手工验收和 SPEC §7 Never list review 为准。

---

## 12. Immediate Next Action

当前 release 收尾已完成。下一步只剩提交/打包/发布流程，不再从本 SPEC 派生执行任务。

**T1.3 `llama-cpp-2` 新依赖触发 §7 Ask First**：首次引入时另起 PR 走依赖审批，论证 feature flag whitelist（`metal` / `cpu`）和 CI 构建矩阵。

**Gate 后移记录**：原 spike 三维门槛曾计划迁移至 release validation；2026-04-24 产品决策已关闭这类长期观察 gate。
